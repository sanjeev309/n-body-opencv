...
int main()
    ...
    Mat disp(Mat(WORLD_MAX_X, WORLD_MAX_Y, CV_8UC3));
    ...
    #pragma omp parallel for schedule(dynamic)
        for(int i=0;i<nBodies;i++){
                ...
                circle(disp,Point(round(p[i].x * scale_x),round(p[i].y * scale_x)),p[i].mass,Scalar(255,255,255),-1);

        }
        putText(disp,"Epoch: "+to_string(iter),Point(WORLD_MAX_X/2 * scale_x,30),FONT_HERSHEY_COMPLEX,0.5,Scalar(255,255,255),1,LINE_8,false);
        imshow("Nbody",disp);
